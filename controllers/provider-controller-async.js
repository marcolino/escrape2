...
locals.fetchListPages = function(providers) {
  var pages = [];
  ...
  async.each(
    ...
    function(err) { // 3rd param is the function to call when everything's done (callback)
      if (err) {
        ...
        console.info('HURRAHHH: doSomethingOnceAllAreDone();');
        locals.fetchDetailsPages(pages);
      }
    }
  );
};

locals.fetchDetailsPages = function(pages) {
console.log('number of pages fetched:', pages.length);
return;
  ...
  async.each(
    ...
    function(err) { // 3rd param is the function to call when everything's done (callback)
      if (err) {
        ...
        // all tasks are successfully done now
        console.info('HURRAHHH 2: doSomethingOnceAllAreDone();');
      }
    }
  );
}
...
/////////////////////////////////////////////////////////////////////////////
      ...
      locals.fetchListPages(providers);
/*
      async.each(
        providers, // 1st param in async.each() is the array of items
        function(provider, callback) { // 2nd param is the function that each item is passed to
          ...
          //provider.someAsyncCall(function() {
