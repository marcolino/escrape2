...
*/
            ...
            post.title = match[4];
/*
            post.date = match[5]; // post date format is dd/mm/yyyy
            post.contents = match[6];
            post.contents = post.contents.replace(/^\s*              /, '');
            post.contents = post.contents.replace(/\s*$/, '');
            post.contents = post.contents.replace(/^(<br>)*          /, '');
            post.contents = post.contents.replace(/(<br>)*$/, '');
            post.beauty = match[7];
            post.attitude = match[8];
            post.sympathy = match[9];
            post.cleanliness = match[10];
            post.site = {};
            post.site.quality = match[11];
            post.site.cleanliness = match[12];
            post.site.reachability = match[13];
*/
